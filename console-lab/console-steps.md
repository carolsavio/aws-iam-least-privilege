...
6 - Cole o JSON com as políticas personalizadas necessárias. → **Save changes.**
>Não se esqueça de trocar **account-id** e colocar o **ARN** do seu usuário administrador atual para não perder o acesso!
...
4 - Na tela de **Permissions** → **Add permissions** e pesquise por:
...
6 - Encontre a role na lista e acesse ela. → Na aba **Permissions** novamente **Add permissions** → **Create inline policy**.

7 - Clique na aba **JSON** e cole a politica em JSON criada.

8 - Finalize com um nome, nesse caso usei `AcessoS3Customizado` → **Create policy**.
...
>Já vem com o Agente do SSM instalado por padrão, poupando trabalho.

4 - **Instance type**: (Escolha qualquer uma que seja elegível ao nível gratuito). No meu caso, utilizei **t3.micro**.

5 - Key pair (login): Escolha **Proceed without a key pair** no menu suspenso.
>Já que a utilizalção será através de SSM, não há necessidade de chaves SSH.
...
9 - Vá para a tela de instâncias, aguarde ela ficar com o status ***Running*** e copie o Instance ID e cole em um bloco de notas (começa com i-xxxxxxxxxxxx). Você vai precisar dele no próximo passo.
...
> **Atenção:** Substitua "region", "account-id" e "i-xxxxxxxxxxxx" pelos dados reais da sua conta e da EC2 criada no Passo 3. → **Next**

3 - Dê o nome para essa politica, no meu caso utilizei `Acesso-SSM-Devs`.

4 - Ainda no IAM → **User groups** → **Create group**.

5 - **User group name**: `Developers`.
>O nome do grupo foi uma escolha pessoal do projeto.

6 - Em **Attach permissions policies**, pesquise pela política criada anteriormente, no meu caso `Acesso-SSM-Devs` → **Create group**.

---
## 5 - O IAM User e o MFA
Por fim, criação do usuário para os testes de validação de segurança.
...
5 - Na tela de permissões, escolha **Add user to group** e selecione o grupo `Developers`. Clique em Next e depois Create user.
